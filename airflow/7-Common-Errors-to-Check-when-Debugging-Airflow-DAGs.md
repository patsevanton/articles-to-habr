...
Вы написали новую группу обеспечения доступности баз данных, которая должна запускаться каждый час. Вы устанавливаете почасовой интервал, начинающийся сегодня в 14:00, и устанавливаете напоминание, чтобы проверить его через пару часов. Вы проверяете его в 15:30 и обнаруживаете, что хотя ваша группа обеспечения доступности баз данных действительно работала, в ваших журналах указано, что существует только одна записанная дата выполнения на 14:00. А что происходило в 3 часа дня?
...
1. По замыслу Airflow DAG будет выполняться по завершении своего schedule_interval.
...
2. Время в Airflow по умолчанию указано в формате UTC.
...
Не следует ожидать, что выполнение ваших DAG-файлов будет соответствовать вашему местному часовому поясу. Если вы находитесь в тихоокеанском времени США, запуск группы DAG с 19:00 будет соответствовать 12:00 по местному времени.
Начиная с версии 1.10, Airflow фактически учитывает часовой пояс, но мы по-прежнему рекомендуем сохранять ваши DAG-файлы с расчетом на временные метки UTC для согласованности.

2. Одна из ваших групп DAG не работает.
Если рабочие процессы в вашем развертывании обычно выполняются без сбоев, но вы обнаруживаете, что одна конкретная группа обеспечения доступности баз данных не планирует задачи и не запускается вообще, это может иметь какое-то отношение к тому, как вы настроили ее для расписания.
Убедитесь, что у вас нет
datetime.now ()
как ваша start_date
Интуитивно понятно, что если вы скажете своей группе DAG начать работу «сейчас», она выполнится «сейчас». НО, это не учитывает, как сам Airflow фактически читает datetime.now ().
Для выполнения DAG start_date должен быть временем в прошлом, иначе Airflow будет считать, что он еще не готов к выполнению. Когда Airflow оценивает ваш файл DAG, он интерпретирует datetime.now () как текущую метку времени (т.е. НЕ время в прошлом) и решает, что он не готов к запуску. Поскольку это будет происходить каждый раз, когда пульс Airflow будет оценивать ваш DAG каждые 5-10 секунд, он никогда не запустится.
...
Ошибка 503 обычно указывает на проблему с веб-сервером вашего развертывания, основным компонентом Airflow, отвечающим за отображение состояния задачи и журналов выполнения задач в интерфейсе Airflow. Если по какой-либо причине у него недостаточно мощности или иным образом возникла проблема, это может повлиять на время загрузки пользовательского интерфейса или доступность веб-браузера.
По нашему опыту, ошибка 503 часто указывает на то, что ваш веб-сервер дает сбой (в Astronomer вы можете услышать, что это называется состоянием CrashLoopBackOff). Если вы запускаете развертывание, и вашему веб-серверу по какой-либо причине требуется больше нескольких секунд для запуска, он может достичь периода ожидания (10 секунд по умолчанию), в котором он вылетит, прежде чем он успеет развернуться. Это вызывает повторную попытку, которая снова дает сбой и так далее.
Если ваше развертывание находится в этом состоянии, возможно, ваш веб-сервер достигает предела памяти при загрузке ваших DAG (даже если ваши рабочие и планировщик продолжают выполнять задачи, как ожидалось).
...
Если вы используете Astronomer, мы рекомендуем поддерживать размер веб-сервера на отметке минимум 5 AU (Astronomer Units). Даже если у вас нет ничего особенно тяжелого, поддержание вашего веб-сервера ниже 5, скорее всего, приведет к некоторому фанковому поведению со всех сторон.
...
Повышение этих значений укажет вашему веб-серверу Airflow подождать немного дольше для загрузки, прежде чем он поразит вас 503 (тайм-аут). Вы все равно можете пробовать медленную загрузку, если ваше развертывание на самом деле недостаточно мощно, но вы, вероятно, избежите попадания в 503.
...
4. Задачи датчиков периодически не работают.
...
Например, если работник может одновременно запускать только X задач, а у вас работает три датчика, то вы сможете запускать только X-3 задачи в любой заданный момент. Имейте в виду, что если вы постоянно используете датчик, это ограничивает то, как и когда может произойти перезапуск планировщика (иначе датчик выйдет из строя).
В зависимости от вашего варианта использования мы предлагаем рассмотреть следующее:

1. Создайте группу доступности базы данных, которая запускается с более частым интервалом.
Возможно, что задан тычок - и пропускает последующие задачи, если файл не найден.
** 2. Запуск лямбда-функции **
...
5. Задачи выполняются, но становятся узкими 
Если все выглядит так, как ожидалось, но вы обнаруживаете, что ваши задачи становятся узкими, мы рекомендуем внимательнее присмотреться к двум вещам:
...
Какие именно эти значения должны быть установлены (и что может стать потенциальным узким местом), зависит от вашей настройки - например, вы запускаете несколько групп DAG одновременно или одну группу DAG с сотнями одновременных задач? С учетом сказанного, их точная настройка, безусловно, может помочь решить проблемы с производительностью. Вот список того, что вы можете найти:

** 1. Параллелизм ** (параллелизм)
Это определяет, сколько экземпляров задач может активно выполняться параллельно в группах обеспечения доступности баз данных с учетом ресурсов, доступных в любой момент времени на уровне развертывания. Думайте об этом как о «максимально активных задачах в любом месте».
ENV AIRFLOW__CORE__PARALLELISM = 18

** 2. Параллелизм DAG ** (dag_concurrency)
Это определяет, сколько экземпляров задач ваш планировщик может запланировать одновременно для каждой группы DAG. Думайте об этом как о «максимальном количестве задач, которые можно запланировать за один раз для каждой группы DAG».
...
** 5. Параллелизм рабочих ** (worker_concurrency)
...
Не путать с приведенными выше настройками. «Параллелизм» здесь устанавливается на индивидуальном уровне группы DAG и определяет количество задач, которые могут выполняться одновременно в одной группе DAG. Это также может потребовать настройки, но это не будет работать, если оно определено как часть файла airflow.cfg.
...
•	Если вы выполняете относительно большое количество легких задач в группах DAG и с относительно высокой частотой, вам, вероятно, лучше иметь 2 или 3 «легких» рабочих для распределения работы.
...
3. Дайте вашим рабочим немного больше прав.
Если вы используете Astronomer, вы можете сделать это на вкладке «Настройка» пользовательского интерфейса Astronomer.
...
7. Задачи медленно планируются или вообще перестали планироваться.
...
•	Запустите astro airflow deploy через интерфейс командной строки для немедленного перезапуска всего (если вы используете Celery, вы можете воспользоваться периодом отсрочки прекращения работника, который вы можете использовать здесь, чтобы свести к минимуму существующие немедленные сбои в выполнении задач) 
