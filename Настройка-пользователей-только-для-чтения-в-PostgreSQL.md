...
Обзор
Перед началом я создал новую схему базы данных под названием myapp, принадлежащую пользователю с именем app-admin. Эта схема включает таблицы для сотрудников, вакансий и клиентов, заполненные фиктивными данными. Я выполнил следующие шаги:

1.	Создал новую роль с именем myapp-readonly.
2.	Предоставил ему разрешение SELECT для таблиц сотрудников и вакансий. Предоставил ему привилегии ограниченного выбора в таблице клиентов, чтобы сохранить конфиденциальность клиентов.
...
Первый оператор дает полное разрешение на чтение таблиц сотрудников и вакансий.
...
Это правило гарантирует, что мои пользователи Redash не увидят эти данные намеренно или нет. Если пользователь попытается выбрать * FROM клиентов, база данных вызовет ошибку разрешений. И в браузере схемы появятся только поля идентификатора и имени.

Шаг 3. Создайте пользователя Redash и назначьте ему роль только для чтения.
Пользователь redash - это то, что мы подключим к экрану настройки источника данных в Redash. Вы должны заменить «секретный» надежным паролем.
...
Наконец, выбирая из таблицы клиентов:
