...
TSBS – это отличный инструмент бенчмаркинга для TSDBs. Он позволяет генерировать произвольное количество метрик, передавая необходимое количество временных рядов, разделенных на 10 флагов масштаба (бывший масштаб-var). 10 – это количество измерений (метрик), генерируемых на каждом узле шкалы. Следующие наборы данных были созданы с помощью TSBS для бенчмарка:
...
Значения InfluxDB (- e необходимы для поддержки высокой мощности. Подробности смотрите в документах):
...
**Уникальные 400К тайм серии (временные ряды)**
...
Показатели Victoria: 2,6М точек данных в секунду; использование оперативной памяти: 3 ГБ; окончательный размер данных на диске: 965 МБ

InfluxDB: 1.2M точек данных в секунду; использование оперативной памяти: 8.5 GB; окончательный размер данных на диске: 1.6 GB

Временная шкала: 849K точек данных в секунду; использование оперативной памяти: 2,5 ГБ; окончательный размер данных на диске: 50 ГБ



Как вы можете видеть из приведенных выше результатов, Victoria Metrics выигрывает в производительности вставки и степени сжатия. Временная шкала выигрывает в использовании оперативной памяти, но она использует много дискового пространства — 29 байт на точку данных.
...
Victoria Metrics использует все доступные vCPUs, в то время как InfluxDB недостаточно использует ~2 из 16 vCPUs.



Timescale использует только 3-4 из 16 vCPUs. Высокие доли iowait и системы на TimescaleDB графике временных масштабов указывают на узкое место в подсистеме ввода-вывода (I/O). Давайте посмотрим на графики использования пропускной способности диска:





Victoria Metrics записывает данные со скоростью 20 Мбит/с с пиками до 45 Мбит/с. Пики соответствуют большим частичным слияниям в дереве LSM.
...
Временная шкала DB (TimescaleDB) ограничена пропускной способностью записи 120 Мбит/с, но иногда она нарушает этот предел и достигает 220 Мбит/с в пиковых значениях. Эти пики соответствуют провалам недостаточной загрузки процессора на предыдущем графике.
...
**4M уникальные тайм серии**



4-метровые временные ряды выглядят немного вызывающе. Но наши конкуренты успешно сдают этот экзамен. Результаты бенчмарка:
...
Производительность InfluxDB упала с 1,2 млн точек данных в секунду для 400-километрового временного ряда до 330 тыс. точек данных в секунду для 4-метрового временного ряда. Это значительная потеря производительности по сравнению с другими конкурентами. Давайте посмотрим на графики использования процессора, чтобы понять первопричину этой потери:
...
TimescaleDB имеет высокую долю системы. Полагаем, что высокая мощность привела ко многим системным вызовам или ко многим незначительным ошибкам страниц.
...
TimescaleDB больше не ограничена диском. Похоже, что он ограничен чем-то еще, связанным с высокой долей системного процессора (CPU).
